...
namespace Wave
{
    ...
    public class WaveOut : IDisposable
    {
        ...
        /// </summary>
        /// <param name="devNumber">Device to test</param>PlaybackState
        ...
        public void Init(Stream waveProvider, WaveFormat waveFormat)
        {
            WaveFormat = waveFormat;

            waveStream = waveProvider;
            int bufferSize = waveFormat.ConvertLatencyToByteSize((DesiredLatency + NumberOfBuffers - 1) / NumberOfBuffers);

            MmResult result;
            lock (waveOutLock)
            {
                result = WaveInterop.waveOutOpen(out hWaveOut, (IntPtr)DeviceNumber, WaveFormat, callback, IntPtr.Zero, WaveInterop.WaveInOutOpenFlags.CallbackFunction);
            }
